    ...
    return (
      <div className="flex h-screen w-full items-center justify-center bg-background text-primary">
        <div className="flex flex-col items-center gap-4">
          ...
          <p className="font-mono text-xl uppercase tracking-widest">Initializing RPA Core...</p>
        </div>
      </div>
    );
  }
    ...
    <div className="flex h-screen w-full bg-background text-foreground overflow-hidden font-sans transition-colors duration-300">
      ...
      <main className="flex-1 overflow-y-auto p-8 space-y-8">
        ...
        <header className="flex justify-between items-end">
          <div>
            <h2 className="text-4xl font-black uppercase tracking-tighter italic">System Overview</h2>
            <p className="text-muted-foreground font-mono">Real-time automation performance metrics</p>
          </div>
          ...
          <Card className="brutalist-card bg-card">
            ...
            <CardContent className="space-y-6">
              <div className="space-y-2">
                ...
                <Progress value={75} className="h-3 bg-muted border border-current" />
              </div>
              <div className="space-y-2">
                <div className="flex justify-between text-sm font-bold uppercase">
                  <span>Pending</span>
                ...
                <Progress value={20} className="h-3 bg-muted border border-current" />
              </div>
              <div className="space-y-2">
                <div className="flex justify-between text-sm font-bold uppercase">
                  <span>Failed</span>
                ...
                <Progress value={5} className="h-3 bg-muted border border-current" />
              </div>

              <div className="pt-4 border-t border-current/20">
                <div className="grid grid-cols-2 gap-4">
                  <div className="p-4 border-2 border-current bg-primary/10 text-center">
                    <p className="text-2xl font-black italic">{summary?.completedToday}</p>
                  ...
                  <div className="p-4 border-2 border-current bg-destructive/10 text-center">
                    <p className="text-2xl font-black italic">{summary?.failedToday}</p>
        ...
        <Card className="brutalist-card bg-card">
          <CardHeader className="flex flex-row items-center justify-between">
            <CardTitle className="uppercase italic tracking-tight">Active Queues</CardTitle>
            <Badge variant="outline" className="border-current text-foreground rounded-none uppercase px-4 py-1">
              Live Feed
            </Badge>
          </CardHeader>
          <CardContent>
            <Table>
              <TableHeader>
                <TableRow className="border-b-2 border-current hover:bg-transparent">
                  <TableHead className="text-foreground font-bold uppercase italic">Queue ID</TableHead>
                  <TableHead className="text-foreground font-bold uppercase italic">Process Name</TableHead>
                  <TableHead className="text-foreground font-bold uppercase italic">Status</TableHead>
                  <TableHead className="text-foreground font-bold uppercase italic">Priority</TableHead>
                  <TableHead className="text-foreground font-bold uppercase italic text-right">Load</TableHead>
                </TableRow>
              </TableHeader>
              <TableBody>
                {queues.map((q) => (
                  <TableRow key={q.id} className="border-b border-current/20 hover:bg-foreground/5 transition-colors">
                    <TableCell className="font-mono text-primary font-bold">{q.id}</TableCell>
                    <TableCell className="font-bold">{q.name}</TableCell>
                    <TableCell>
                      <Badge 
                        className={`rounded-none uppercase px-3 ${
                          q.status === "Active" ? "bg-primary text-black" : 
                          q.status === "Error" ? "bg-destructive text-white" : 
                    ...
                    <TableCell>
                      <span className={`text-xs font-bold uppercase ${
                        q.priority === "High" ? "text-accent" : "text-muted-foreground"
                      }`}>
                        {q.priority}
                      </span>
                    </TableCell>
